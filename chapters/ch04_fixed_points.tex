...
\subsection{Comparison of the Three Canonical Examples}

Table~\ref{tab:three_examples} summarizes the three examples and their roles in the RG framework.

\begin{table}[h!]
\centering
\renewcommand{\arraystretch}{1.4}
\caption{The three canonical examples and the RG concepts they illustrate.}
\label{tab:three_examples}
\begin{tabular}{@{}p{2.8cm}p{3.5cm}p{3.5cm}p{3.5cm}@{}}
\toprule
\textbf{Feature} & \textbf{Oscillator} & \textbf{$\phi^4$ Theory} & \textbf{PME} \\
\midrule
\textbf{Equation} & $\ddot{x} + \omega^2 x + \epsilon x^3 = 0$ & $Z = \int e^{-\int(\frac{1}{2}(\nabla\phi)^2 + \frac{r}{2}\phi^2 + \frac{u}{4!}\phi^4)}$ & $\partial_t \rho = \nabla^2(\rho^m)$ \\
\textbf{Scale parameter} & Time $t$ & Cutoff $\Lambda$ & Time $t$ \\
\textbf{Running parameter(s)} & Frequency $\omega(t)$, amplitude $A(t)$ & Mass $r(\Lambda)$, coupling $u(\Lambda)$ & Exponents $\alpha(m)$, $\beta(m)$ \\
\textbf{Beta function} & $\beta_\omega = \frac{3\epsilon A^2}{8\omega}$ & $\beta_r = 2r + O(u)$, $\beta_u = -\epsilon u + O(u^2)$ & Implicit via scaling analysis \\
\textbf{Fixed points} & None (continuum of states) & Gaussian ($u^*=0$), Wilson-Fisher ($u^* \sim \epsilon$) & Self-similar solution \\
\textbf{Anomalous dim.} & None ($\gamma = 0$) & $\eta = O(\epsilon^2)$ & $\gamma \neq 0$ generically \\
\textbf{Borel structure} & Alternating $n!$, instanton & Renormalon at $\zeta = k/\beta_1$ & TBD \\
\textbf{Key lesson} & Secular terms $\to$ running & Beta functions, universality & Second-kind self-similarity \\
\bottomrule
\end{tabular}
\end{table}

Each example adds complexity while remaining analytically tractable. The oscillator has no fixed points but shows the basic mechanism of running parameters. The $\phi^4$ theory has non-trivial fixed points but anomalous dimensions only at two loops. The PME has anomalous dimensions at leading order, illustrating second-kind self-similarity.
