...
QED demonstrates the six-step RG recipe in its original domain: relativistic quantum field theory. The scale hierarchy (Step 1) extends from the electron mass to the UV cutoff. Perturbative analysis with Borel structure (Step 2) reveals renormalon singularities. Theory space (Step 3) is parametrized by the fine structure constant and electron mass, extended to include non-perturbative sectors. The beta function (Step 4), equation~\eqref{eq:qed_beta}, describes charge screening by vacuum polarization. Fixed-point analysis (Step 5) reveals that the Gaussian fixed point $\alpha^* = 0$ is an IR attractor. Physical predictions to high precision require Borel resummation (Step 6).
