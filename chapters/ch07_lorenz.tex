...
The Lorenz system provides our first extended application of the RG framework developed in Part I. Originally derived as a simplified model of atmospheric convection, the Lorenz equations exhibit multi-scale behavior that makes them a natural testing ground for RG methods.

We apply the geometric RG framework systematically. First, we identify the temporal scale hierarchy created by the parameter $\sigma$, which separates fast velocity relaxation from slow temperature evolution. Second, we define coarse-graining through the slow manifold reduction. Third, we construct a theory space parametrized by the control parameters $\sigma$, $\rho$, and $\beta$. Fourth, we derive the beta functions for slowly varying amplitudes near bifurcation points. Fifth, we analyze the fixed-point landscape, including both the simple fixed points of the flow and the strange attractor viewed as a ``fixed set.''
...
Following the analysis of the anharmonic oscillator in the Prologue, we can derive beta functions for slowly varying amplitudes. Near the onset of chaos, the amplitude $A$ of oscillations around the unstable fixed points evolves according to
\begin{equation}
...
\subsection{Period-Doubling as Discrete RG}

\marginnote{The period-doubling RG is a \emph{discrete} transformation on function space, not a continuous flow. This makes it closer in spirit to Wilson's block-spin RG than to continuous RG flows.}

The Feigenbaum analysis provides a beautiful example of the RG paradigm applied to dynamical systems. Following Sethna's exposition, we make the RG structure explicit.

\textbf{The map space.} Consider the space of unimodal maps $f: [0,1] \to [0,1]$ with a single maximum. Examples include the logistic map $f_r(x) = rx(1-x)$ and the sine map $f(x) = a\sin(\pi x)$.

\textbf{The doubling operator.} Define the RG transformation $\mathcal{R}$ by:
\begin{equation}
(\mathcal{R}f)(x) = \alpha f(f(x/\alpha))
\end{equation}
where $\alpha$ is chosen so that $(\mathcal{R}f)$ has the same normalization as $f$ (e.g., maximum value 1 at $x = 0$).

Geometrically, $\mathcal{R}$ rescales and composes: it looks at the second iterate $f \circ f$ near the maximum, rescaled to the original domain.

\textbf{The fixed point.} There exists a unique (up to rescaling) fixed point $f^*$ satisfying:
\begin{equation}
\mathcal{R}f^* = f^*, \qquad f^*(0) = 1, \quad (f^*)'(0) = 0
\end{equation}

This fixed-point map is \emph{universal}---it is the same for the logistic map, the sine map, or any other unimodal map approaching period-doubling accumulation.

\begin{workedbox}[Box 9.1: The Feigenbaum Fixed Point and Scaling Constant]
\textbf{The fixed-point equation:}
\begin{equation}
f^*(x) = \alpha f^*(f^*(x/\alpha))
\end{equation}

The constant $\alpha = -2.5029\ldots$ is the spatial rescaling factor.

\textbf{Stability analysis:} Linearize $\mathcal{R}$ around $f^*$:
\begin{equation}
\mathcal{R}(f^* + \epsilon g) = f^* + \epsilon \mathcal{L}g + O(\epsilon^2)
\end{equation}
where $\mathcal{L}$ is a linear operator on the space of perturbations $g$.

\textbf{The relevant eigenvalue:} The operator $\mathcal{L}$ has one eigenvalue greater than 1:
\begin{equation}
\boxed{\delta = 4.66920\ldots}
\end{equation}

This is the \textbf{Feigenbaum constant}. It measures how fast perturbations grow under the RG.

\textbf{Physical interpretation:}
\begin{itemize}
\item The eigenvalue $\delta > 1$ means there is one \textbf{relevant direction} in map space
\item Maps with $\mu < \mu_\infty$ flow toward stable cycles; maps with $\mu > \mu_\infty$ flow toward chaos
\item The ratio of parameter intervals is: $\frac{\mu_{n} - \mu_{n-1}}{\mu_{n+1} - \mu_n} \to \delta$
\end{itemize}

\textbf{Comparison to Wilson-Fisher:}
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
& \textbf{Period-Doubling} & \textbf{3D Ising} \\
\hline
Fixed point & $f^*$ (universal map) & Wilson-Fisher $g^*$ \\
Relevant eigenvalue & $\delta = 4.669\ldots$ & $1/\nu \approx 1.587$ \\
Scaling law & $\mu_n - \mu_\infty \sim \delta^{-n}$ & $\xi \sim |T - T_c|^{-\nu}$ \\
Universality class & Unimodal maps & $\mathbb{Z}_2$ symmetry \\
\end{tabular}
\end{center}
\end{workedbox}

\textbf{The second Feigenbaum constant.} The spatial rescaling factor $\alpha = -2.5029\ldots$ is the second universal constant. It measures how the amplitude of oscillations scales between successive period-doublings.

\textbf{Universality.} The constants $\delta$ and $\alpha$ are universal: they take the same values for \emph{any} smooth unimodal map. This is because all such maps, under successive applications of $\mathcal{R}$, flow to the same fixed point $f^*$.

This is the deepest form of universality: not just the same exponents, but the same \emph{function} describing the dynamics at criticality.
...
\begin{enumerate}
\item \textbf{Stability at the origin.} For the Lorenz system with parameters $(\sigma, \rho, \beta)$:
\begin{enumerate}
\item Compute the eigenvalues of the stability matrix at the origin.
\item Show that one eigenvalue crosses zero at $\rho = 1$.
\item Determine the stability type (node, saddle, spiral) for $\rho$ slightly less than and greater than 1.
\end{enumerate}

\item \textbf{Convective fixed points.} For $\rho > 1$:
\begin{enumerate}
\item Verify that $C^\pm = (\pm\sqrt{\beta(\rho-1)}, \pm\sqrt{\beta(\rho-1)}, \rho-1)$ are fixed points.
\item Compute the stability matrix at $C^+$.
\item Show that the eigenvalues become complex for sufficiently large $\sigma$.
\end{enumerate}

\item \textbf{Slow manifold.} When $\sigma \gg 1$:
\begin{enumerate}
\item Use the ansatz $x = y + \epsilon x_1$ with $\epsilon = 1/\sigma$ to derive the first correction.
\item Show that the leading slow dynamics is $\dot{y} = y(\rho - 1 - z)$, $\dot{z} = y^2 - \beta z$.
\item Discuss how this reduction relates to integrating out fast modes.
\end{enumerate}

\item \textbf{Feigenbaum scaling.} For the logistic map $x_{n+1} = rx_n(1-x_n)$:
\begin{enumerate}
\item Find the values $r_1, r_2, r_3$ where period-doubling bifurcations occur.
\item Estimate the Feigenbaum ratio $\delta = (r_{n-1} - r_{n-2})/(r_n - r_{n-1})$.
\item Explain why $\delta$ is universal across different maps.
\end{enumerate}

\item \textbf{(Challenge) Lyapunov exponents.} For the Lorenz attractor:
\begin{enumerate}
\item Derive the variational equations for small perturbations $\delta\mathbf{x}$.
\item Explain why there must be one zero Lyapunov exponent.
\item Discuss how the sum of Lyapunov exponents relates to phase space contraction.
\end{enumerate}
\end{enumerate}
...
\begin{solutionbox}{Exercise 9.1: Stability at the Origin}
\textbf{(a) Eigenvalues of stability matrix.}

The stability matrix at the origin is:
\begin{equation}
B = \begin{pmatrix} -\sigma & \sigma & 0 \\ \rho & -1 & 0 \\ 0 & 0 & -\beta \end{pmatrix}
\end{equation}

The characteristic polynomial factors as $(\lambda + \beta)\det(B' - \lambda I)$ where $B'$ is the upper-left $2\times 2$ block:
\begin{equation}
(\lambda + \beta)[(\lambda + \sigma)(\lambda + 1) - \sigma\rho] = 0
\end{equation}

Eigenvalues:
\begin{equation}
\lambda_1 = -\beta, \qquad \lambda_{2,3} = \frac{-(\sigma+1) \pm \sqrt{(\sigma+1)^2 + 4\sigma(\rho-1)}}{2}
\end{equation}

\textbf{(b) Eigenvalue crossing at $\rho = 1$.}

At $\rho = 1$: $\lambda_{2,3} = \frac{-(\sigma+1) \pm (\sigma+1)}{2}$, giving $\lambda_2 = 0$ and $\lambda_3 = -(\sigma+1)$.

The discriminant is $(\sigma+1)^2 + 4\sigma(\rho-1)$. For $\rho < 1$, this is still positive (since $\sigma > 0$), but both roots are negative. For $\rho > 1$, one root becomes positive.

\textbf{(c) Stability type.}

For $\rho < 1$: All three eigenvalues are real and negative $\Rightarrow$ \textbf{stable node}.

For $\rho$ slightly $> 1$: One positive, two negative eigenvalues $\Rightarrow$ \textbf{saddle point} (unstable).

The transition at $\rho = 1$ is a \textbf{pitchfork bifurcation} where the origin loses stability and two new fixed points $C^\pm$ appear.
\end{solutionbox}

\begin{solutionbox}{Exercise 9.2: Convective Fixed Points}
\textbf{(a) Verify fixed points.}

Substitute $C^+ = (\sqrt{\beta(\rho-1)}, \sqrt{\beta(\rho-1)}, \rho-1)$ into the Lorenz equations:
\begin{align}
\dot{x} &= \sigma(y - x) = \sigma(\sqrt{\beta(\rho-1)} - \sqrt{\beta(\rho-1)}) = 0 \checkmark \\
\dot{y} &= x(\rho - z) - y = \sqrt{\beta(\rho-1)}(\rho - (\rho-1)) - \sqrt{\beta(\rho-1)} = 0 \checkmark \\
\dot{z} &= xy - \beta z = \beta(\rho-1) - \beta(\rho-1) = 0 \checkmark
\end{align}

\textbf{(b) Stability matrix at $C^+$.}

Let $a = \sqrt{\beta(\rho-1)}$. The Jacobian is:
\begin{equation}
J = \begin{pmatrix} -\sigma & \sigma & 0 \\ 1 & -1 & -a \\ a & a & -\beta \end{pmatrix}
\end{equation}

\textbf{(c) Complex eigenvalues.}
...
For large $\sigma$, the eigenvalues become complex conjugate pairs when the discriminant of the reduced cubic becomes negative. The Hopf bifurcation occurs at:
\begin{equation}
\rho_c = \sigma\frac{\sigma + \beta + 3}{\sigma - \beta - 1}
\end{equation}

For $\sigma = 10$, $\beta = 8/3$: $\rho_c = 10 \times \frac{10 + 8/3 + 3}{10 - 8/3 - 1} \approx 24.74$.
\end{solutionbox}

\begin{solutionbox}{Exercise 9.3: Slow Manifold}
\textbf{(a) First correction.}

From $\dot{x} = \sigma(y - x)$, when $\sigma \gg 1$, $x$ rapidly adjusts to $y$. Let $x = y + \epsilon x_1$ where $\epsilon = 1/\sigma$:
\begin{equation}
\dot{y} + \epsilon\dot{x}_1 = \sigma(y - y - \epsilon x_1) = -x_1
\end{equation}

At leading order, $\dot{y} = -x_1$, so:
\begin{equation}
x_1 = -\dot{y} = -[y(\rho - z) - y] = y(1 + z - \rho)
\end{equation}

Thus $x = y + \frac{y(1 + z - \rho)}{\sigma} + O(1/\sigma^2)$.

\textbf{(b) Leading slow dynamics.}

Setting $x = y$ in the $y$ and $z$ equations:
\begin{align}
\dot{y} &= y(\rho - z) - y = y(\rho - 1 - z) \\
\dot{z} &= y^2 - \beta z
\end{align}

This is a two-dimensional system on the slow manifold.

\textbf{(c) Relation to RG.}

The fast variable $x$ is ``integrated out'' by averaging over its rapid relaxation. The effective slow dynamics involves renormalized parameters. This is analogous to integrating out high-energy modes in QFT, producing an effective action for the slow (IR) degrees of freedom.
\end{solutionbox}

\begin{solutionbox}{Exercise 9.4: Feigenbaum Scaling}
\textbf{(a) Period-doubling values.}

For the logistic map $x_{n+1} = rx_n(1-x_n)$:
\begin{itemize}
\item $r_1 = 3$: Period-1 to period-2 bifurcation
\item $r_2 \approx 3.449$: Period-2 to period-4
\item $r_3 \approx 3.5441$: Period-4 to period-8
\item $r_4 \approx 3.5644$: Period-8 to period-16
\end{itemize}

\textbf{(b) Feigenbaum ratio estimate.}

\begin{equation}
\delta \approx \frac{r_2 - r_1}{r_3 - r_2} = \frac{3.449 - 3}{3.5441 - 3.449} = \frac{0.449}{0.0951} \approx 4.72
\end{equation}

\begin{equation}
\delta \approx \frac{r_3 - r_2}{r_4 - r_3} = \frac{0.0951}{0.0203} \approx 4.68
\end{equation}

The exact Feigenbaum constant is $\delta = 4.669201...$

\textbf{(c) Universality explanation.}

The RG acts on the space of unimodal maps. The period-doubling transformation $T$ takes a map $f$ to $f \circ f$ (suitably rescaled). The fixed point $f^*$ satisfies $T[f^*] = f^*$.

The eigenvalue of $DT$ at this fixed point is $\delta$. Since the RG transformation depends only on the qualitative shape (unimodal with quadratic maximum), not on specific coefficients, $\delta$ is universal across all such maps.
\end{solutionbox}

\begin{solutionbox}{Exercise 9.5: Lyapunov Exponents (Challenge)}
\textbf{(a) Variational equations.}

For $\mathbf{x} = (x, y, z)^T$ satisfying the Lorenz equations, consider a perturbation $\delta\mathbf{x}$:
\begin{equation}
\frac{d}{dt}\delta\mathbf{x} = J(\mathbf{x}(t))\,\delta\mathbf{x}
\end{equation}
where $J$ is the time-dependent Jacobian along the trajectory:
\begin{equation}
...
\begin{equation}
\lambda_i = \lim_{t\to\infty} \frac{1}{t}\ln\|\delta\mathbf{x}_i(t)\|
\end{equation}
where $\delta\mathbf{x}_i$ are orthogonalized perturbation vectors.

\textbf{(b) Zero Lyapunov exponent.}

Consider the perturbation along the trajectory: $\delta\mathbf{x} = \dot{\mathbf{x}}$.

By differentiating the Lorenz equations:
\begin{equation}
\frac{d}{dt}\dot{\mathbf{x}} = J(\mathbf{x})\dot{\mathbf{x}}
\end{equation}

This perturbation neither grows nor shrinks exponentially (it just moves along the trajectory), giving $\lambda = 0$.

This is a consequence of time-translation symmetry: shifting the initial time produces a trajectory that stays the same distance from the original.

\textbf{(c) Phase space contraction.}

The sum of Lyapunov exponents equals the time-averaged divergence of the flow:
\begin{equation}
\sum_i \lambda_i = \langle \nabla \cdot \mathbf{F} \rangle = \langle -\sigma - 1 - \beta \rangle = -(\sigma + 1 + \beta)
\end{equation}

For $\sigma = 10$, $\beta = 8/3$: $\sum \lambda_i = -13.67$.

Since the sum is negative, phase space volume contracts exponentially, forcing trajectories onto an attractor of zero volume (measure zero). The attractor is ``strange'' because it has fractal dimension, filling more than a surface but less than a volume.
\end{solutionbox}
...
\begin{summarybox}{Chapter 9: Chaotic Dynamics -- The Lorenz System}

\summaryheader{RG Framework Applied}
\begin{itemize}
\item \textbf{Scale hierarchy:} Fast ($\sim 1/\sigma$) vs slow variables; amplitude vs phase
\item \textbf{Coarse-graining:} Slow manifold reduction averages out fast dynamics
\item \textbf{Theory space:} $(\sigma, \rho, \beta)$ or amplitude/phase coordinates
\item \textbf{Beta functions:} $\frac{dA}{ds} = \lambda A - \kappa A^3$ near bifurcations
\item \textbf{Fixed points:} Origin, $C^\pm$, strange attractor as ``fixed set''
\item \textbf{Physical predictions:} Universal scaling at onset of chaos
\end{itemize}

\summaryheader{Key Physical Insights}
\begin{itemize}
\item \textbf{Lyapunov exponents} play role of scaling dimensions: $\lambda > 0$ (relevant), $\lambda = 0$ (marginal), $\lambda < 0$ (irrelevant)
\item \textbf{Strange attractor} supports invariant measure analogous to fixed-point theory
\item \textbf{Feigenbaum universality:} $\delta = 4.669...$ is universal across all unimodal maps
\item \textbf{Routes to chaos} are RG universality classes
\end{itemize}

\summaryheader{Chaos--Phase Transition Analogy}
\begin{itemize}
\item Lyapunov exponent $\leftrightarrow$ Order parameter
\item Onset of chaos $\leftrightarrow$ Critical point
\item Characteristic time $\tau \sim |\mu - \mu_c|^{-\nu}$ $\leftrightarrow$ Correlation length divergence
\end{itemize}
