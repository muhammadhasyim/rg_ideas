...
The Lorenz system demonstrates how the five-step RG recipe applies to chaotic dynamics. We identified the temporal scale hierarchy created by large $\sigma$ (Step 1) and defined coarse-graining through slow manifold reduction (Step 2). The theory space is parametrized by the control parameters $\sigma$, $\rho$, and $\beta$ (Step 3). The beta functions for slowly varying amplitudes, equation~\eqref{eq:lorenz_amplitude}, describe the evolution near bifurcation points (Step 4). The fixed-point analysis (Step 5) revealed both simple fixed points of the flow and the strange attractor as a ``fixed set'' with invariant measure.
